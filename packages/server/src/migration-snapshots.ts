export const MIGRATION_001_RECORD_TYPES = [
...
export const MIGRATION_001_LINK_TYPES = [
...
const terms = (vocabulary: string, packageSource: string, values: readonly string[], extensionAllowed = false, seededOther = false) =>
  values.map((term) => ({ vocabulary, term, packageSource, extensionAllowed, seededOther: seededOther || term === "other" }));

export const MIGRATION_001_VOCABULARY_TERMS = [
  ...
  ...terms("repair_operation", "docs/worldbuilding-system/13_contradiction_retcon_and_mystery.md", ["clarify scope", "retcon", "reinterpret", "institutionalize", "branch", "quarantine", "supersede", "deprecate", "reject"]),
