...
export class WorldStore {
  ...
  startPropagationRun(input: { factRecordId: number; debtRecordId?: number }): unknown {
    const fact = this.getRecord(input.factRecordId);
    if (input.debtRecordId != null) {
      const debt = this.getRecord(input.debtRecordId);
      if (debt.recordTypeKey !== "canon_debt") throw new Error("propagation debt must be a canon debt record");
      if (!this.propagationQueue().some((row) => row.id === debt.id)) throw new Error("debt is not an open propagation-scoped debt item");
    }
  ...
  getPropagationRun(flowId: number): unknown {
    const flow = this.propagationFlow(flowId);
    return {
      flow,
      plan: this.propagationPlan(flow.propagation_fact_record_id),
      consequences: this.propagationConsequences(flowId),
      domainSweeps: this.propagationDomainSweeps(flowId),
      dispositions: this.propagationDispositions(flowId),
      proposals: this.db.prepare("SELECT * FROM propagation_surfaced_proposals WHERE flow_id = ? ORDER BY id").all(flowId)
    };
  }
  ...
  addPropagationConsequence(input: { flowId: number; orderKey: string; domainName?: string; body: string; pressure?: "normal" | "high" }): PropagationConsequenceRow {
    ...
    this.db.prepare("UPDATE flow_instances SET current_step = ? WHERE id = ?").run(`propagation:${order[0]}`, input.flowId);
    return rowToPropagationConsequence(this.db.prepare("SELECT * FROM propagation_consequences WHERE id = ?").get(result.lastInsertRowid) as Record<string, unknown>);
  }
  ...
  recordPropagationDomain(input: { flowId: number; domainName: string; triage: "direct" | "dependency" | "reaction" | "negative"; declaration?: string }): PropagationDomainSweepRow {
    this.propagationFlow(input.flowId);
    ...
    this.db.prepare("UPDATE flow_instances SET current_step = 'propagation:domain-atlas' WHERE id = ?").run(input.flowId);
    const row = result.lastInsertRowid
      ? this.db.prepare("SELECT * FROM propagation_domain_sweeps WHERE id = ?").get(result.lastInsertRowid)
      : this.db.prepare("SELECT * FROM propagation_domain_sweeps WHERE flow_id = ? AND domain_name = ?").get(input.flowId, input.domainName);
  ...
  dispositionPropagationConsequence(input: { consequenceId: number; disposition: string; note?: string; debtName?: string; preservationBoundary?: string }): PropagationDispositionRow {
    const consequence = this.db.prepare("SELECT * FROM propagation_consequences WHERE id = ?").get(input.consequenceId) as Record<string, unknown> | undefined;
    if (!consequence) throw new Error(`Propagation consequence not found: ${input.consequenceId}`);
    this.assertVocabularyTerm("consequence_disposition", input.disposition);
    ...
    if (input.disposition === "assigned as canon debt") {
      ...
      debtRecordId = this.createCanonDebt({ name: input.debtName, scope: "propagation", assignee: "steward", body: input.note ?? String(consequence.body) }).id;
    }
    ...
    this.db.prepare("UPDATE flow_instances SET current_step = 'propagation:disposition' WHERE id = ?").run(Number(consequence.flow_id));
    return rowToPropagationDisposition(this.db.prepare("SELECT * FROM propagation_consequence_dispositions WHERE id = ?").get(result.lastInsertRowid) as Record<string, unknown>);
  }
  ...
  skipPropagationStep(input: { flowId?: number; stepKey: string; admissionLevel?: string; workScale?: string; reason?: string }): RecordRow {
    ...
    if (input.flowId != null) {
      const flow = this.propagationFlow(input.flowId);
      this.createLink(record.id, flow.propagation_fact_record_id, "derived_from", "Propagation step declined");
      this.db.prepare("UPDATE flow_instances SET current_step = ? WHERE id = ?").run(`propagation:skip:${input.stepKey}`, input.flowId);
    }
    return record;
  }
  ...
  closePropagationRun(flowId: number): { flow: unknown; report: RecordRow; debt: RecordRow | null; missing: PropagationConsequenceRow[] } {
    const flow = this.propagationFlow(flowId);
    const missing = this.undispositionedHighPressureConsequences(flowId);
    if (missing.length) {
    ...
    return this.db.transaction(() => {
      const report = flow.propagation_report_record_id == null
        ? this.writePropagationReport(flowId)
        : this.getRecord(flow.propagation_report_record_id);
      const fact = this.getRecord(flow.propagation_fact_record_id);
      this.createLink(fact.id, report.id, "digest_of", "Fact card shock-cone digest points to propagation report");
      this.db.prepare("UPDATE propagation_surfaced_proposals SET report_record_id = ? WHERE flow_id = ? AND report_record_id IS NULL").run(report.id, flowId);
      const proposalRows = this.db.prepare("SELECT proposal_record_id FROM propagation_surfaced_proposals WHERE flow_id = ?").all(flowId) as Array<{ proposal_record_id: number }>;
      for (const row of proposalRows) {
        this.createLink(row.proposal_record_id, report.id, "derived_from", "Fact surfaced by propagation report");
      }
      const debt = flow.propagation_debt_record_id == null ? null : this.closeCanonDebt(flow.propagation_debt_record_id);
      this.db.prepare("UPDATE flow_instances SET state = 'complete', current_step = 'propagation:complete', propagation_report_record_id = ? WHERE id = ?").run(report.id, flowId);
      return { flow: this.db.prepare("SELECT * FROM flow_instances WHERE id = ?").get(flowId), report, debt, missing: [] };
    })();
  }

  correctPropagationReport(input: { originalReportId: number; title?: string; body: string }): RecordRow {
    const original = this.getRecord(input.originalReportId);
    ...
    return this.db.transaction(() => {
      const correction = this.createRecord({
      ...
      this.createLink(correction.id, original.id, "supersedes", "Propagation report correction; original remains append-only");
      return correction;
    })();
  }
  ...
  private propagationFlow(flowId: number): { id: number; state: string; current_step: string; propagation_fact_record_id: number; propagation_debt_record_id: number | null; propagation_report_record_id: number | null } {
    const flow = this.db.prepare("SELECT * FROM flow_instances WHERE id = ? AND flow_key = 'propagation'").get(flowId) as Record<string, unknown> | undefined;
  ...
  private propagationConsequences(flowId: number): PropagationConsequenceRow[] {
    return this.db.prepare("SELECT * FROM propagation_consequences WHERE flow_id = ? ORDER BY id").all(flowId).map((row) => rowToPropagationConsequence(row as Record<string, unknown>));
  }

  private propagationDomainSweeps(flowId: number): PropagationDomainSweepRow[] {
    return this.db.prepare("SELECT * FROM propagation_domain_sweeps WHERE flow_id = ? ORDER BY id").all(flowId).map((row) => rowToPropagationDomainSweep(row as Record<string, unknown>));
  }

  private propagationDispositions(flowId: number): PropagationDispositionRow[] {
    return this.db.prepare(`
      SELECT d.*
  ...
  private undispositionedHighPressureConsequences(flowId: number): PropagationConsequenceRow[] {
    return this.db.prepare(`
      SELECT c.*
  ...
  private writePropagationReport(flowId: number): RecordRow {
    const flow = this.propagationFlow(flowId);
    const fact = this.getRecord(flow.propagation_fact_record_id);
    const consequences = this.propagationConsequences(flowId);
    const domainSweeps = this.propagationDomainSweeps(flowId);
    const dispositions = this.propagationDispositions(flowId);
    ...
    const proposals = this.db.prepare(`
      SELECT r.*
      FROM propagation_surfaced_proposals p
      JOIN records r ON r.id = p.proposal_record_id
      WHERE p.flow_id = ?
      ORDER BY p.id
    `).all(flowId).map((row) => rowToRecord(row as Record<string, unknown>));
    const body = [
    ...
    this.replaceSections(report.id, [
      ...
      { heading: "Stopping-rule audit", body: `High-pressure consequences: ${consequences.filter((row) => row.pressure === "high").length}\nUndispositioned high-pressure consequences: ${this.undispositionedHighPressureConsequences(flowId).length}\nStates: answered; intentionally scoped out; assigned as canon debt; protected as a mystery boundary.`, position: 8 }
    ]);
    this.createLink(report.id, fact.id, "derived_from", "Propagation report works this fact's shock cone");
    for (const disposition of dispositions) {
      if (disposition.debtRecordId != null) this.createLink(report.id, disposition.debtRecordId, "requires_follow_up", "Propagation consequence assigned as canon debt");
    }
    return report;
  }
