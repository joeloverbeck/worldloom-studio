...
export class WorldFile {
  ...
  findInProgressContradictionFlowBySource(sourceRecordId: number): FlowInstanceRow | null {
    return (this.db.prepare(`
      SELECT * FROM flow_instances
  ...
  insertContradictionImplicatedRecord(flowId: number, recordId: number): void {
    this.db.prepare("INSERT OR IGNORE INTO contradiction_implicated_records (flow_id, record_id) VALUES (?, ?)").run(flowId, recordId);
  }

  contradictionImplicatedRecordIds(flowId: number): number[] {
    return (this.db.prepare("SELECT record_id FROM contradiction_implicated_records WHERE flow_id = ? ORDER BY id").all(flowId) as Array<{ record_id: number }>)
      .map((row) => row.record_id);
  }

  insertContradictionTitle(flowId: number, title: string): void {
    this.db.prepare(`
  ...
  contradictionWorkScale(flowId: number): string | null {
    const row = this.db.prepare("SELECT work_scale FROM contradiction_work_scales WHERE flow_id = ?").get(flowId) as { work_scale: string } | undefined;
    return row?.work_scale ?? null;
  }

  contradictionDisposition(flowId: number): { disposition: string; note: string } | null {
    const row = this.db.prepare("SELECT disposition, note FROM contradiction_dispositions WHERE flow_id = ?").get(flowId) as { disposition: string; note: string } | undefined;
    return row ?? null;
  }

  contradictionTriageEntries(flowId: number): Array<{ step_key: string; body: string }> {
    return this.db.prepare("SELECT step_key, body FROM contradiction_triage_entries WHERE flow_id = ? ORDER BY id").all(flowId) as Array<{ step_key: string; body: string }>;
  }

  upsertContradictionTriageEntry(flowId: number, stepKey: string, body: string): FlowInstanceRow {
    this.db.prepare(`
      INSERT INTO contradiction_triage_entries (flow_id, step_key, body)
      VALUES (?, ?, ?)
      ON CONFLICT(flow_id, step_key) DO UPDATE SET body = excluded.body
    `).run(flowId, stepKey, body);
    return this.db.prepare("SELECT * FROM contradiction_triage_entries WHERE flow_id = ? AND step_key = ?").get(flowId, stepKey) as FlowInstanceRow;
  }

  upsertContradictionWorkScale(flowId: number, workScale: string): FlowInstanceRow {
    this.db.prepare(`
    ...
    return this.db.prepare("SELECT * FROM contradiction_work_scales WHERE flow_id = ?").get(flowId) as FlowInstanceRow;
  }

  upsertContradictionDisposition(flowId: number, disposition: string, note?: string): FlowInstanceRow {
    this.db.prepare(`
    ...
    return this.db.prepare("SELECT * FROM contradiction_dispositions WHERE flow_id = ?").get(flowId) as FlowInstanceRow;
  }

  contradictionRepairOperations(flowId: number): Array<{ operation: string; repair_text: string; position: number }> {
    return this.db.prepare("SELECT operation, repair_text, position FROM contradiction_repair_operations WHERE flow_id = ? ORDER BY position").all(flowId) as Array<{ operation: string; repair_text: string; position: number }>;
  }

  replaceContradictionRepairOperations(flowId: number, input: { operations: string[]; repairText: string }): Array<{ operation: string; repair_text: string; position: number }> {
    this.db.prepare("DELETE FROM contradiction_repair_operations WHERE flow_id = ?").run(flowId);
    ...
    return this.contradictionRepairOperations(flowId);
  }

  contradictionRepairTargets(flowId: number): Array<{ record_id: number; next_canon_status: string; new_title: string | null; new_body: string | null; note: string; advisory_record_id: number | null }> {
    return this.db.prepare("SELECT record_id, next_canon_status, new_title, new_body, note, advisory_record_id FROM contradiction_repair_targets WHERE flow_id = ? ORDER BY id").all(flowId) as Array<{ record_id: number; next_canon_status: string; new_title: string | null; new_body: string | null; note: string; advisory_record_id: number | null }>;
  }

  insertContradictionRepairTarget(input: { flowId: number; recordId: number; nextCanonStatus: string; newTitle?: string; newBody?: string; note?: string; advisoryRecordId?: number }): FlowInstanceRow {
    const result = this.db.prepare(`
    ...
    return this.db.prepare("SELECT * FROM contradiction_repair_targets WHERE id = ?").get(result.lastInsertRowid) as FlowInstanceRow;
  }

  insertContradictionRepairCreatedProposal(input: { flowId: number; proposalRecordId: number; reportRecordId?: number | null }): void {
    this.db.prepare(`
  ...
  contradictionRepairCreatedProposals(flowId: number): FlowInstanceRow[] {
    return this.db.prepare("SELECT * FROM contradiction_repair_created_proposals WHERE flow_id = ? ORDER BY id").all(flowId) as FlowInstanceRow[];
  }

  assignContradictionReportToCreatedProposals(flowId: number, reportRecordId: number): void {
    this.db.prepare("UPDATE contradiction_repair_created_proposals SET report_record_id = ? WHERE flow_id = ? AND report_record_id IS NULL").run(reportRecordId, flowId);
  }

  contradictionRetconCosts(flowId: number): Array<{ retcon_type: string; cost_key: string; cost_text: string }> {
    return this.db.prepare("SELECT retcon_type, cost_key, cost_text FROM contradiction_retcon_costs WHERE flow_id = ? ORDER BY id").all(flowId) as Array<{ retcon_type: string; cost_key: string; cost_text: string }>;
  }

  replaceContradictionRetconCosts(flowId: number, input: { retconType: string; costs: Array<{ cost: string; text: string }> }): Array<{ retcon_type: string; cost_key: string; cost_text: string }> {
    this.db.prepare("DELETE FROM contradiction_retcon_costs WHERE flow_id = ?").run(flowId);
    ...
    return this.contradictionRetconCosts(flowId);
  }

  contradictionRepairPropagation(flowId: number): FlowInstanceRow | null {
    return (this.db.prepare("SELECT * FROM contradiction_repair_propagation WHERE flow_id = ?").get(flowId) as FlowInstanceRow | undefined) ?? null;
  }

  upsertContradictionRepairPropagation(input: { flowId: number; action: "assign" | "decline"; debtRecordId: number | null; skipRecordId: number | null; note: string }): FlowInstanceRow {
    this.db.prepare(`
    ...
    return this.contradictionRepairPropagation(input.flowId)!;
  }

  completedMysteryChecklistForFlow(flowId: number): boolean {
    return Boolean(this.db.prepare("SELECT 1 FROM mystery_preservation_checklists WHERE flow_id = ? AND completed = 1 LIMIT 1").get(flowId));
  }

  owedBoundaryRows(): FlowInstanceRow[] {
    return this.db.prepare(`
      SELECT
  ...
  insertMysteryBoundaryLink(propagationDispositionId: number, ledgerRecordId: number): void {
    this.db.prepare(`
  ...
  insertMysteryPreservationChecklist(input: { flowId?: number; ledgerRecordId?: number; protectedRecordId?: number; operation: string; effectType: string; body: string; sacredGuardBody?: string }): FlowInstanceRow {
    const result = this.db.prepare(`
    ...
    return this.db.prepare("SELECT * FROM mystery_preservation_checklists WHERE id = ?").get(result.lastInsertRowid) as FlowInstanceRow;
  }

  mysteryPreservationChecklistsForFlow(flowId: number): FlowInstanceRow[] {
