...
The app context: a local-first, single-steward web app implementing the Causal Canon Worldbuilding System (version 1.1) as guided flows over a structured record store.
...
**Prompt-out step**:
A flow step, attached to a decision point, that generates a self-contained prompt — in the package's proposal or pressure mode — the steward may run in any external LLM; always optional.
_Avoid_: AI step, generation step

**Decision point**:
A flow step where the steward must judge, author, classify, route, skip, or dispose of material under package doctrine. One decision point covers one coherent block of steward-authored material — never a single form field, never a whole flow.
...
**Workflow map**:
The home surface of an open world: the method journey rendered against live world state — stages done, active, owed, blocked, or not yet earned, the queues holding work, and the next decision. Guided flows are destinations entered from it.
_Avoid_: dashboard, progress tracker, project board
