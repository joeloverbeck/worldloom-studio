...
23. **Prompt-out evidence-list review.** At the Proposal/Pressure packet-review decision point, source-manifest and omission rows use the structured canonical evidence-list contract from `docs/specs/prompt-out-context-assembly.md`. Equivalent bounded paths aggregate with stable identity, ordered provenance, and a visible count; different candidates, evidence kinds, rules, relationships, standings, or decision meanings remain distinct even when display text matches. The governing package sources remain `09_temporal_and_timeline_protocol.md` and `20_ai_assisted_workflow.md`. Required, optional, skippable, and severity-dependent obligations; app-owned doctrine; governed decline and its retained reason threshold; incomplete and stale blockers with exact remediation; selected mode; advisory/canon separation; current, next, resume, and safe-return orientation; and all read-only guarantees remain unchanged.
24. **Source-selected guided-flow run entry.** Temporal run entry consumes the shared server-owned contract in `docs/specs/workflow-map-and-navigation.md`, downstream of methodology `docs/worldbuilding-system/08_constraint_composition.md`, `docs/worldbuilding-system/09_temporal_and_timeline_protocol.md`, and `docs/worldbuilding-system/12_institutional_economic_and_suppression_protocol.md`. Fact, capability, canon-debt, selected-material, and pass-report modes remain unchanged. Before record-backed start or resume, the decision point shows current short ID, title, record type, canon status, stable numeric id, requested source type, validation state, and any obligation/report/destination binding. It renders empty, resolving, resolved, missing, incompatible, unavailable-standing, stale-binding, and mismatched-binding states with server-owned blocker, remediation, substance rule, preserved input, and action availability. Selected material keeps title/body identity without fabricated record or Conditional-pass metadata. Start/resume revalidates the stable selection, repeats it in the loaded run, adopts refreshed descriptive fields for the same id, refuses identity discontinuity or retargeting, and preserves current/next/resume plus fresh-map safe return without read-side mutation.
...
| Prompt-out evidence-list review and skips | `09`, `20`, W-1, W-4, and ADR 0007 | Review canonical source-manifest and omission evidence, then use, dispose, cite, or decline optional advisory Temporal support. | yes | Equivalent paths aggregate; meaningful tuple differences remain distinct; major skipped work requires a reason and may create debt. Incomplete or stale work is refused with remediation. | Proposal and Pressure use the Spatial-temporal analyst contract; selected mode, advisory/canon separation, and current/next/resume/safe-return orientation remain visible. |
...
The run-entry decision point treats all source identity and validity fields as required for record-backed modes, retains the pass-report identity as resume context, introduces no new optional or severity-dependent authoring field, and leaves governed Conditional-pass deferral on the Workflow map as the existing skippable path. Package paths are provenance; app-owned Temporal entry doctrine explains the selected source, why it is valid, and why a bound pass is owed.

## Browser Acceptance Scenarios

At least one representative scenario shows entry or resume, trigger recommendation, current decision, doctrine at point of use, required/optional/skippable/severity-dependent obligations, server blocker/substance validation, staged coverage and lineage, Prompt-out preview and advisory separation, canonical evidence-list review with aggregate and same-text distinctions, a card/proposal/debt/skip or explicit non-mutation, final/correction write intent, read-side trail, safe exit, and resume state.
