...
## Source-Selected Guided-Flow Run-Entry Evidence Pattern

This pattern is downstream of methodology `08_constraint_composition.md`, `09_temporal_and_timeline_protocol.md`, and `12_institutional_economic_and_suppression_protocol.md` and the **Source-selected guided-flow run entry** contract in `docs/specs/workflow-map-and-navigation.md`.

The docs-naive cognitive walkthrough starts on a fresh production Workflow map with the outstanding `temporal_timeline` obligation for `FAC-3 · Non-reproducible neurological access` and final `PRP-1` Propagation report. In one active browser instance it follows the server-returned destination, verifies before start that short ID and title are primary while record type, canon status, stable numeric id, requested source type, validation state, Conditional-pass reason, obligation, report, destination, and binding validity remain readable supporting context, and identifies why the source is valid without opening repository documents.

The same walkthrough then enters one missing, wrong-type, unavailable, stale, or mismatched selection; observes the exact adjacent server blocker, remediation, and applicable substance validation; confirms the entered id and type remain present, start/resume is disabled, the state change is announced, focus remains on the correctable control, and keyboard recovery needs no pointer-only action; corrects the selection; and observes the same authoritative resolved identity rather than browser reconstruction.

Start or resume must repeat the stable identity approved at entry. The walkthrough observes refreshed descriptive fields for that same id, proves an existing run cannot be retargeted or duplicated by a different selection, and treats an actual identity discontinuity as a visible refusal. Safe return waits for a fresh Workflow-map response before current, next, resume, obligation, and Admission orientation render. The same run spot-confirms the shared human-first identity grammar at Constraint Composition and Institutional / Economic / Suppression entry, including a record-section or pass-report identity where supported and the unchanged selected-material title/body path.

Accessibility evidence names the input, human identity, validation state, blocker, remediation, and start/resume action as one connected decision context; records live announcements and predictable focus; and confirms keyboard operation. Console evidence records errors and warnings separately across navigation, invalid recovery, start/resume, safe return, and both sibling confirmations, with no unexplained entry. Screenshots or equivalent durable observations identify each material state.

Before/after World-file fingerprints cover runs, revisions, reports, obligation projection/events, records, links, debt, skips, advisory artifacts, Admission contents and ordering, canon standing, and unrelated flows. They prove resolution, navigation, safe return, and rejected actions are read-only; only the separately approved existing start/resume action may write its specified flow state. The evidence also records package provenance, app-owned doctrine, required/optional/skippable/severity-dependent classifications, source/report/obligation provenance, blocker text, and current/next/resume orientation at the decision boundary.

Focused HTTP and React evidence supports the walkthrough by covering direct numeric resolution, all three destination compatibility rules, selected material, record-section and pass-report variants where supported, missing and wrong-type records, unavailable standing, stale and mismatched binding, title refresh, existing-run resume, exact remediation, input preservation, action availability, announcements, focus, and non-mutation. Source-policy guards may prove that identity, compatibility, binding, currentness, remediation, and routes are consumed from server responses, but they never replace the rendered production route.
