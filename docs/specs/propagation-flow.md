...
17. **Open-run staging lifecycle.** Consequences and domain declarations remain editable staging while the run is open. Each revision lineage has at most one `active` version; retired versions are `superseded` when replaced and `retracted` when removed without replacement. Closing freezes that staging lifecycle and writes the append-only report-regime record.
18. **Audit-safe revision.** Revise and retract actions use stable row identity, require a steward-authored reason, retain the prior content and triage state, and record prior-version lineage plus actor, timestamp, and flow-step provenance. Revision never silently overwrites staged prose and never creates a generalized cross-flow revision mechanism.
19. **Version-bound dispositions and coverage.** A consequence disposition governs exactly the consequence version it names. Superseding a consequence leaves that disposition in history while the active replacement begins undispositioned; retracting a consequence removes the retired version and its disposition from active-close calculation. Order coverage, domain coverage, blockers, readiness, and preview are calculated only from active versions and current active dispositions.
20. **Active-set currentness.** The server owns a Propagation active-set identity that changes after consequence, domain, or disposition mutation. Prompt-out packets carry that identity. A packet over an older identity is stale, names the changed row or revision in recovery data, refuses packet-bound actions, and offers a server-provided current Proposal or Pressure packet path.
21. **Pressure-currentness obligation.** When a used Pressure beat is followed by substantive revision, support becomes owed again. The server offers a fresh Pressure packet or the governed Prompt-out skip path; the existing major-or-higher reason threshold applies and external LLM use never becomes mandatory.
22. **Atomic action contract.** Revision, retraction, re-disposition, and packet-bound actions are server-owned and atomic. A failure identifies the affected row and concrete remediation, returns the unchanged current run, and gives the browser enough recovery data to preserve entered material without inferring policy.
23. **Finalization boundary.** Close preview identifies final active rows, retained revision audit, source/debt effects, links and queues to be written, and canon, Admission work, sibling proposals, unrelated debt, and advisory artifacts left untouched. Close writes one report containing the final active shock cone and relevant lineage/reasons. Every staging revision action is refused after close; correction creates a new report and never reopens or mutates the closed run or report.

## Pre-Close Revision and Finalization Decision Contract

The named decision contract is **Pre-close Propagation revision and finalization**. It governs the period after the steward has staged shock-cone or domain-atlas material and before the append-only `propagation_report` exists. `docs/worldbuilding-system/07_propagation_engine.md` supplies the shock-cone, coverage, stopping, and report doctrine; `docs/worldbuilding-system/20_ai_assisted_workflow.md` supplies Proposal/Pressure, adoption-as-authorship, and optional external assistance. The package files are provenance; the browser carries an app-owned derivation of the operative rule at the affected row and close control.

Open-run material and the closed report are different mutation regimes:

- **Staging:** consequence and domain versions may be revised or retracted only while the run is open. Retired content remains readable through lineage and audit data but cannot satisfy active coverage or enter close as active material.
- **Final report:** successful close serializes the active rows and relevant revision audit into one append-only `propagation_report`. It does not convert staged rows into independently canonical facts, change source standing, admit surfaced proposals, or rewrite prior reports.

Consequence revision returns the retired version, its historical disposition if any, the active replacement, the invalidated active-disposition state, the lineage, active-set identity, blockers, readiness, preview, packet currentness, doctrine/write intent, and read-side audit. Domain revision returns the retired declaration and triage state, the active replacement or retraction, recomputed active-only atlas coverage, and the same current decision envelope. Cross-run targets, retired targets, missing reasons, invalid replacements, and every post-close attempt are refused without partial writes.

The read side distinguishes three things explicitly: the final active result, the open or frozen staging lineage that explains how it changed, and the append-only report record. Superseded or retracted prose may appear as audit history but is never labeled as active output.
...
| Pre-close revision and finalization | `07_propagation_engine.md` and `20_ai_assisted_workflow.md` | Keep, revise, supersede, or retract staged consequence/domain versions; then restore active-only coverage and current support before close. | yes | Active severity obligations are recomputed after every mutation; substantive revision after used Pressure owes fresh Pressure or the governed skip. | Proposal and Pressure packets carry the active-set identity; stale packets are refused and recover through the server-provided current packet path. |
...
- **Revision/finalization:** active, superseded, and retracted rows are visibly distinct and retain stable lineage, reason, and provenance. Row-local revise/retract controls show required replacement/reason fields and severity-dependent obligations. Returned historical dispositions, active-only blockers, stale/current packet explanation, fresh-Pressure-or-skip recovery, doctrine, write intent, close preview, and inline failure remediation remain server-owned. Failed actions preserve all entered values.
...
No new methodology-facing record types are introduced. This flow uses existing `canon_fact`, `canon_debt`, `propagation_report`, `skip_record`, `advisory_artifact`, `canon_change_proposal`, `flow_instances`, `record_links`, `record_sections`, `record_facets`, `jurisdiction_events`, and prompt-template tables. Flow-owned staging fields or revision rows may carry stable lineage, version, lifecycle, reason, provenance, and active-set identity as specified by `schema-v1.md`; they are not a reusable cross-flow revision subsystem.
...
Primary seam: localhost HTTP API against temp-file world databases for owed queue, run start/resume, consequence writing, domain and negative-domain declarations, consequence/domain revise and retract, invalidated readiness, stale/current Prompt-out actions, fresh Pressure or governed skip, proposal routing, refused close, successful close, report/readback, post-close refusal, digest links, and debt closure.

Secondary seam: direct store/SQL assertions for one-active-version-per-lineage, reason/provenance persistence, version-bound dispositions, active-only coverage, active-set identity, migration of existing open runs, transaction atomicity, report append-only enforcement, debt minting, link integrity, proposal jurisdiction, and consequence-disposition validation.

## Principles

Touches `charter.md` (P-3, P-4, T-8), `canon-sovereignty.md` (P-2, W-1), `domain-fidelity.md` (P-1, T-2), `workflow-principles.md` (P-5, W-2, W-3, W-4, W-7), `guided-workflow-usability.md` (W-8, W-9), `data-principles.md` (P-6, W-5, W-6, T-3, T-4, T-5), and ADRs 0001-0004 and 0007-0009. It affirms non-contradiction: the server owns staging revision and close policy; shared Prompt-out owns packet lifecycle; the browser renders returned governance state; every surviving word stays steward-authored; external LLM use remains optional; sweeps propose and Admission admits; report-regime records remain append-only master audit records.
