...
- For Temporal/Timeline Prompt-out, the packet includes the current Temporal decision, source record or selected material, the active staged coverage revision, stable revision/lineage/active-set identity, all ten steward-authored lenses, date facets, first-true and first-known timing, latency, residue, sequence-integrity and retrospective-insertion answers, temporal mystery boundaries, relevant retired revision audit explicitly labeled as history, retained/final/correction report relationships, existing timeline cards, routed proposals, canon debt, skips, advisory dispositions, `09` doctrine, source manifest, explicit omissions, and advisory/canon warning. Superseded coverage is never presented as active decision material.
...
## Temporal Active-Set Currentness

Temporal packet identity consumes the server-owned active-set identity and active revision returned by the open `temporal_timeline` run. A material coverage replacement advances that identity, names the changed revision and reason in recovery data, and stales every represented generate, copy, download, advisory storage, disposition, advisory-use, and skip action. The shared Prompt-out lifecycle refuses stale actions atomically and returns the current Temporal packet route, active revision, active-set identity, blockers, preserved mode, and current/next/resume orientation.

When a Pressure packet was used before a material revision, Temporal marks Pressure support owed again. The steward may load and use fresh Pressure over the revised active identity or record the governed Prompt-out skip under the existing severity-dependent reason rule. This is an audit/currentness obligation, not mandatory external LLM use. Temporal owns the revision and obligation state; Prompt-out continues to own shared packet generation, preview, copy/download, advisory, disposition, use, skip validation, and recovery mechanics.
