...
- Generate an acceptance manifest from saved exact issue JSON with `.claude/skills/implement/scripts/build-acceptance-manifest.mjs`, include every reviewed parent/child issue, and pass it as `--acceptance-manifest <path>` whenever `--child-family` is used. The validator rejects missing issues, unexpected issues, and missing AC/Principles IDs.
...
- In every zero-residual child coverage row, add `sequence:` to the `Acceptance source`: name the ordered events and the proof that observes their order, or use `sequence: N/A because ...` when order cannot affect acceptance. Independently observed states are not evidence for a required transition order.
- When no child-family table applies, add `Spec sequence coverage: sequence: <ordered events plus observing proof / N/A because ...>` before reporting zero Spec findings.
...
Spec sequence coverage: sequence: <ordered events plus observing proof / N/A because the reviewed acceptance is not sequence-sensitive; optional when every child row above carries sequence>
