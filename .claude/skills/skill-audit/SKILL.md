...
6. **Severity-tag each finding** — CRITICAL / HIGH / MEDIUM / LOW:
   ...
   **Implementation-handoff baseline** — in a Git worktree, immediately before presenting the report and after the final finding reads, capture these identities sequentially rather than in parallel: `git rev-parse HEAD`, `git status --short -- <covered paths>`, and a SHA-256 identity for the target `SKILL.md`, every target reference/template/script cited by a finding, and each alignment document whose content affects a finding. Use the repository-available SHA-256 command, such as `sha256sum` or `shasum -a 256`. Include the compact block from the report template below so a later Step 8 can measure change rather than infer it from conversation memory. Outside Git, record HEAD/status as N/A and still capture file identities when the environment supports it.
...
|---|---|---|
| <section-prefixed finding> | <implemented / relocated / cascade / co-edit / skipped> | <location, anchor that satisfies it, and rationale> |
