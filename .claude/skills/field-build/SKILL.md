...
4. Record the exact subagent id or canonical handle returned by the tool, save the raw output, and close or release the probe as usual.
...
- **Open a run-scoped live log and evidence dirs** under `/tmp/worldloom-field-build/`. Choose the report `<NN>` and `<world-slug>` before the first browser action, then use `build-log-<NN>-<world-slug>.md` for this run's append-only raw log, `screenshots/` for browser screenshots, and `cold-llm/` for verbatim prompt packets and cold-subagent outputs. Never append a new run to another run's log; after a compaction or interruption within the same run, reuse its exact run-scoped log and append a resume checkpoint. These scratch artifacts are the build's evidence trail and resume point, not the deliverable — polish belongs in the report. Keep screenshots and cold-LLM artifacts out of the repo unless the user explicitly asks to commit evidence. Initialize this log before the first browser action; do not drive the UI until it records the seed, baseline `git status --short`, current HEAD, report number/slug, regression-set source, mandatory/opportunistic regression IDs, prior-art surfaces checked or unreachable, app/server URL plan, and user-directed evidence targets. Before writing the final report, reconcile this run's live log through the final frontier so it names every resumed action, final decision point, and stop reason.
...
3. Reopen the canonical world and **read back that it is unmutated** before continuing the build. Do not resume authoring until that readback passes.
4. State in the live log and report **which probes ran on a copy**, so no reader mistakes copy evidence for canonical world state.
...
- Prompt-out coverage: proposal=<active-surface state + prompt path, raw/artifact/handoff hashes and byte lengths, output path, subagent id, or reason; include `diagnostic=exercised via <route>` when applicable>; pressure=<same>
...
Then run the executable closeout validator after the three audit tables exist:
...
Pass `--no-prior-report` only when this run genuinely had no prior canonical report. Record the command and its output in the Closeout run sheet. A nonzero exit stops closeout; revise the report or live log, rerun the manual audit affected by the failure, and rerun the validator before cleanup.
