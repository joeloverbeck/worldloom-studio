...
3. Dispatch through a mechanism that accepts those verified packet bytes directly and exposes a submitted-payload digest/readback, then record its SHA-256 and byte length and match them to the active raw packet. A digest computed only from text that was manually retyped or pasted into an opaque tool call does not prove the handoff. If the dispatch surface cannot prove what it submitted, record `probe unavailable — exact handoff unverified` and do not count the answer as cold-probe evidence.
...
3. **Pressure.** With the field now authored, generate the **pressure** prompt (challenge mode) on that material and confirm its preview. Save the packet verbatim at `/tmp/worldloom-field-build/cold-llm/field-build-<NN>-<decision-slug>-pressure-prompt.md`; complete the end-to-end cold-handoff proof, then hand that verified saved packet to a newly spawned fresh cold LLM and record the tool-returned id/handle and handoff hash. Save the raw cold answer verbatim at `/tmp/worldloom-field-build/cold-llm/field-build-<NN>-<decision-slug>-pressure.md`, close or release the subagent once the output is saved, and cite both paths plus the subagent identity in the live log. Read the challenge — missing prerequisites, contradictions, off-tone drift, mystery damage — and revise the field where the challenge earns it. If the exact packet cannot be extracted, its handoff cannot be proved, or a fresh subagent is unavailable, record `probe unavailable` with the reason; if the app offers no pressure mode where authored material exists, log P and record `blocked by app` in the coverage ledger.
   ...
   - **Unrevisable-material stop.** If Pressure earns a substantive change but the active workflow exposes no edit, retract, supersede, or replace path for the staged material, do not append compensating contradictions and do not close or finalize the run. Log a **blocking structural F** with the absent controls and read-side state as evidence. Record any server/UI `ready` result as **mechanical close readiness** and record **methodology readiness** separately as blocked. Safe-return to the last committed stable surface, park the Frontier at the blocked revision, list the exact changes to replay after correction becomes possible, and mark the pressure loop `blocked by app`; do not run another cold pass against unchanged invalid material.
...
**Drive the UI; never infer a flow works from its code or tests.** Screenshot each screen into `/tmp/worldloom-field-build/screenshots/`; name shots `field-build-<NN>-<shot>-<slug>.png` so they do not collide across runs, and record the names in the log. **Cold-LLM subagents get the verified saved packet and nothing else** — that isolation *is* the packet-context test; leaking world or doc context into them destroys the finding. Do not use an already-contextual subagent for a later cold probe. If a true fresh subagent/tool or verifiable exact handoff is unavailable, do not simulate the probe with the main agent's context; mark the prompt test `probe unavailable`, log Q if the packet exists but the probe is unavailable, log P if the app failed to supply the packet, and carry that limitation into the report.
...
- **The cold LLM stays cold.** Its only input is the exact app-generated packet saved for that decision and mode, proven through the end-to-end cold-handoff chain. The moment world context, the docs, this conversation, a summary, a pre-contextual subagent, or an opaque unverified handoff enters the probe, the packet-context finding vanishes and the report lies. If you cannot extract the exact packet, prove the submitted bytes, or get a genuinely fresh subagent, mark the probe `probe unavailable` rather than answering it yourself.
