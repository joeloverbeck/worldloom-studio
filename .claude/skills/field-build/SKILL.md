...
- Prompt-out coverage: proposal=<exercised/refused/blocked/probe unavailable/deferred + prompt path, output path, subagent id, or reason>; pressure=<same>
...
Before finalizing, run this checklist against the report. This is a hard closeout gate, not a reminder. If any required item is missing, revise the report before cleanup or final response.
