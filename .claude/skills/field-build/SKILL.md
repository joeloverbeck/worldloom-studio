...
For every boundary-state entry, add a nested `Boundary proof:` line. A fixed verdict names the passed pre-boundary assertion and the passed after-boundary negative control explicitly; a not-reverifiable verdict names the passing side, the unreachable side, and the blocking finding ID. The executable closeout validator rejects a boundary fixed verdict without both passed assertions.
