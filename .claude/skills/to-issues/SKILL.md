...
Make that self-check visible before publication. For every affected `ready-for-agent` issue, keep a short local mapping in the draft notes, approval checkpoint, or run sheet that marks each applicable checklist item as `covered by AC <n>` or `covered by "<short staged criterion excerpt>"`, or `N/A - <reason>`. Do not publish the mapping as a required issue-body section unless house style already does; the point is to force a concrete coverage pass before the label is applied. In the final ledger or verification report, include `checklist mapped: yes` for each affected issue, or `checklist mapped: N/A - <reason>` for unaffected issues, so checklist coverage is not silently replaced by section/placeholder readback; that final `yes` is a summary of the concrete local mapping, not a substitute for it.
...
2. For any `ready-for-agent` / `ready-for-human` issue subject to the browser-visible guidance checklist, rerun the final checklist mapping against the staged acceptance criteria and record each applicable item as `covered by AC <n>`, `covered by "<short staged criterion excerpt>"`, or `N/A - <reason>` in draft notes or the run sheet.
...
For multi-issue publications, an optional run sheet can keep the serial gates legible. Keep it local-only and advisory; it is not a required output. Useful columns: `slice`, `body file`, `placeholder tokens`, `final checklist mapping`, `same-run reference sanity`, `blocked by`, `created issue`, `sweep ok`, `verify ok`, and `substituted into`.
