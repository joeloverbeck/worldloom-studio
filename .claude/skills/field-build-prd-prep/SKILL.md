...
Complete intake only when the source report path, any existing same-stem prep artifact, any applicable named-prior-run prep artifact, baseline branch, and baseline worktree dirt are known.
...
4. Check the default same-stem prep path with a proofable readback, such as `if test -e <path>; then echo "exists"; else echo "missing"; fi` or `ls -l <path>`. Always record `Existing same-stem prep artifact classification:` in the output header, using `missing at intake` when no prior artifact exists. If `reports/<field-build-report-stem>-prd-prep.md` exists, read it and classify it as current, partially consumed, stale, superseded, or not relevant.
   ...
   If the source report names a prior run and carries, reproduces, confirms, or extends one of that run's findings or app seeds, check the prior report's default same-stem prep path too. Do not recurse beyond the one named prior run. Build a bounded prior-run carry-forward ledger containing only overlapping identities; mark each `consumed`, `still live`, or `rejected` from current source and tracker evidence, and include the prior first operational action, selected PRD or program, follow-ons, and coverage items only when they own or sequence an overlapping identity. Keep this ledger separate from the current report's same-stem classification, and summarize its result in the reassessment verdict.
...
When a bounded prior-run carry-forward ledger exists, apply the same packaging rules to its still-live overlapping rows. Do not inherit the prior run's first-PRD choice, follow-on status, or sequencing unless current evidence independently selects it.
...
1. Run `node .claude/skills/field-build-prd-prep/scripts/validate-prd-prep.mjs <source-report> <prep-artifact>`. Fix structural errors and inspect every warning. Compare the helper's reported finding, app-seed, regression, and frontier counts with the corresponding source sections; a zero or lower count that omits finding-like source entries is not a clean pass. The helper checks coverage and output shape only; it does not decide whether a finding's semantic classification or recommended scope is correct. If the helper rejects source-report syntax rather than artifact structure, or silently undercounts a non-empty source category, do not edit the source report or this skill during the prep. Confirm the source remains unchanged, compare the rejected or omitted form with the current canonical field-build report grammar, run the helper unit tests plus a read-only compatibility probe, and classify the failure as `artifact defect` or `validator defect`. Fix artifact defects in the prep. For a validator defect, record the canonical failure or undercount and diagnostic probe in the completion self-check, route the helper repair through `skill-audit`, and continue only when the read-only probe proves the artifact's coverage and shape.
