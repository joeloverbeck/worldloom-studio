...
The app's purpose is to let a steward follow the whole worldbuilding methodology with the `docs/worldbuilding-system/*` files closed. This skill measures how far short of that it falls, including the `20_ai_assisted_workflow.md` version 1.1 two-mode Prompt-out constitution: proposal mode for labeled candidate material and pressure mode for challenge on steward-authored material. It runs two tracks in lockstep through the new-world path:
...
- **Resume or seed the trial world:**
  ...
  - Later runs: reopen the same world (the app is local-first; data persists). Resume from the prior report's **Frontier** block and the trial world's visible flow state/admission queue; do not treat frontier discovery as the acceptance task. If a since-shipped fix reset or broke it, re-seed and note that.
...
4. **Prompt-out** — does the app expose the shared two-mode Prompt-out contract for this decision point? Exercise **proposal mode** where it is available or capture the server-owned blocker where it is refused; then exercise **pressure mode** where steward-authored material exists or capture the blocker/omission where it is unavailable. Read each prompt packet: does it gather the *exact* world context this decision needs to be answered well in an external LLM — no more, no less? If a prompt is materially incomplete or wrong, capture the prompt text plus DOM/network evidence, log it as a prompt-out finding, and do not answer or store advisory material unless a minimal answer is still needed to keep the walk moving. If a packet is usable, then, as the external LLM, answer it to produce the field content. *(→ P)*
