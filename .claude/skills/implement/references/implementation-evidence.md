...
When the repo `tdd` skill is invoked for implementation work, carry its closeout evidence forward into the implementation ledger or final closeout audit, not only into review-fix blocks. The durable sink should include the `TDD closeout preflight` fields and the full `TDD evidence gate passed: durable sink ...` line, including compact table/header, seams accounted for, pre-red recovery status, `CONTEXT.md` status, ADRs/principles/docs status, partial-red or red-first skip reasons, evidence-only rows, evidence-only browser console state when browser/manual evidence-only rows exist, and existing-test contract-change row status. If no runnable seam exists for an issue, name that issue and reason in the evidence-only or skip-reason fields instead of implying TDD coverage.
