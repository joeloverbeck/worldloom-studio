...
### Structured closeout hydration

After the final evidence is known, use `--values-input <values.json>` to hydrate every non-audit placeholder deterministically instead of patching a large scaffold by hand. The option requires an explicit completed `--audit-input`; the builder preserves the validated audit block byte-for-byte and applies values only before and after it. The JSON schema is exactly:

```json
{
  "version": 1,
  "values": {
    "<exact placeholder emitted by this scaffold>": "final non-empty replacement"
  }
}
```

Generate the scaffold once without `--values-input`, enumerate every non-audit angle-placeholder key emitted for the selected flags, and provide each key exactly once. When source placeholders repeat, the builder makes each occurrence addressable with an emitted ordinal suffix such as `[1 of 2]`; preserve that full emitted key so different issue rows and gate fields can receive different values. The top-level object accepts only `version` and `values`. The builder rejects a missing key, an unknown key at either level, a blank/non-string value, a version other than `1`, or a replacement that leaves another non-audit placeholder. On success it changes the status line to `Scaffold status: hydrated from --values-input; validate before publication.` The completed body still owes every applicable TDD, review, implement, size, visual-inspection, and tracker-readback gate.

```bash
node .claude/skills/implement/scripts/build-closeout-body.mjs /tmp/worldloom-acceptance-manifest.json --audit-input /tmp/worldloom-acceptance-audit.md --values-input /tmp/worldloom-closeout-values.json --output /tmp/worldloom-closeout-369.md --parent 369 --review normal --immediate-fix --tdd-parent-rollup --browser --principles --fixed-child pending
```

### Chunk-first first-mutation carrier

If the size plan reports `low-headroom` or `exceeds-limit`, stop before filling or tracker mutation. Do not recover space with circular evidence such as `exact named items in this criterion` or by removing required atoms and proof surfaces. Split the detailed audit into disjoint issue/check groups with repeatable `--select <issue[:check-id[,check-id...]]>` arguments on `build-acceptance-manifest.mjs`; each invocation emits a mechanically matching subset manifest and audit scaffold through its normal `--output` and `--audit-output` flags. A selector containing only an issue number includes every check for that issue; a selector with check IDs includes only those IDs, in source order.

Because supplemental comments must exist before the compact parent can cite their stable URLs, designate the lowest-numbered supplemental issue as the **first-mutation carrier**. Use this order:

1. Fill the carrier body with its exact subset audit plus the shared final-SHA, verification, TDD, review, browser/manual, evidence-identity, Principles/ADR, and closeout-preflight evidence required by the family.
2. Run every applicable TDD and review closing validator against the carrier and its subset manifest, then run the implement closing validator with `--closing --expected-final-sha "$(git rev-parse HEAD)" --emit-preflight` plus the applicable browser, principles, fixed-child, review-route, and subset-manifest flags. Copy the emitted preflight verbatim before mutation.
3. Post and exact-read the carrier first. It is a closing-validated supplemental body, not an audit-only chunk.
4. Run `--audit-only --review-entry --acceptance-manifest <subset>` against every remaining supplemental chunk, then post and exact-read those chunks.
5. Insert every stable chunk URL into the compact parent body. Keep the parent's own audit rows disjoint; do not pass them as a `--chunk` tuple because the compact parent's tracker URL does not exist before publication.
6. Run `verify-split-closeout-family.mjs` to prove that the compact rows plus supplemental subset manifests are disjoint and collectively exhaustive, each supplemental subset matches its exact body, and the compact body cites every stable chunk URL. Validate and publish the compact closing body only against its own rows.

This carrier sequence satisfies the global first-tracker-mutation gate while preserving the chunk-first requirement. This chunk-first branch overrides the parent-first 4+ child default in [child-family-closeout.md](child-family-closeout.md).

```bash
node .claude/skills/implement/scripts/build-acceptance-manifest.mjs /tmp/worldloom-issues.json --select 391:US3,Parent-Implementation-Decisions,Parent-Testing-Decisions --output /tmp/worldloom-acceptance-part-01.json --audit-output /tmp/worldloom-acceptance-part-01.md
node .claude/skills/implement/scripts/build-acceptance-manifest.mjs /tmp/worldloom-issues.json --select 392 --output /tmp/worldloom-acceptance-part-02.json --audit-output /tmp/worldloom-acceptance-part-02.md
node .claude/skills/implement/scripts/verify-split-closeout-family.mjs --full-manifest /tmp/worldloom-acceptance-manifest.json --compact-body /tmp/worldloom-closeout-391.md --chunk /tmp/worldloom-acceptance-part-01.json /tmp/worldloom-audit-part-01.md https://github.com/owner/repo/issues/391#issuecomment-1001 --chunk /tmp/worldloom-acceptance-part-02.json /tmp/worldloom-audit-part-02.md https://github.com/owner/repo/issues/391#issuecomment-1002
```
...
Findings: <none / terminal Standards findings only>

## Spec

Sources reviewed: <exact issue/PRD/acceptance sources>
Findings: <none / terminal Spec findings only>
...
- Proof-process finalization: <stdout/stderr and browser console drained after the final assertion; proof-owned processes/sessions stopped or intentionally retained with reason; no HMR errors; no stale-process errors; no unexplained errors / N/A because no proof-owned process or session was started / blocked because ...>
...
node .claude/skills/implement/scripts/validate-closeout-body.mjs "$body" --closing --expected-final-sha "$(git rev-parse HEAD)" --browser --principles --local-only --fixed-child-pending --acceptance-manifest /tmp/worldloom-acceptance-manifest.json
rg -n "<[^>\n]{1,120}>" "$body"
...
rg -n "browser smoke|browser evidence|Console state|Browser console state|Final freshness delta|Proof-process finalization|Fixed child inline close comment|Fixed child final inline close comment inspected" "$body"
...
node .claude/skills/implement/scripts/validate-closeout-body.mjs "$body" --closing --expected-final-sha "$(git rev-parse HEAD)" --emit-preflight --browser --principles --local-only --fixed-child-pending --acceptance-manifest /tmp/worldloom-acceptance-manifest.json
