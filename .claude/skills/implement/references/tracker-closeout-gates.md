...
Everything-active closeout recipe: for 4+ child issues using a parent PRD rollup without supplemental chunks, a local-only final SHA, invoked TDD, local code-review fallback, fixed-template child comments, and browser/manual proof, validate the inspected parent body before the first tracker mutation with:
...
node .claude/skills/implement/scripts/validate-closeout-body.mjs "$body" --closing --expected-final-sha "$(git rev-parse HEAD)" --emit-preflight --browser --principles --local-only --fixed-child-pending --review-fallback --acceptance-manifest /tmp/worldloom-acceptance-manifest.json
```
...
Chunk-first first-mutation carrier: when stable supplemental URLs must exist before the compact parent can be completed, the lowest-numbered supplemental issue carries the first mutation. Its body contains its exact subset audit plus the family's shared closing evidence, passes every applicable TDD/review closing validator against that subset, and passes the implement closing validator with `--emit-preflight`. Copy the emitted preflight, post and exact-read that carrier first, then post the remaining audit-only chunks. In this branch, every “before the first tracker mutation” parent-body instruction applies to the carrier until the compact parent exists; the compact parent is still validated later against only its own rows and all exact-read chunk URLs. See [closeout-templates.md](closeout-templates.md) and [child-family-closeout.md](child-family-closeout.md) for the ordered sequence.
...
- When oversized evidence is split, `verify-split-closeout-family.mjs` has passed against the full manifest, the compact body's own audit rows, and every supplemental subset manifest/body/stable-URL tuple after the chunks were posted and exact-read. This proves their disjoint, collectively exhaustive coverage and compact-body citation of every chunk.
- Every large-body excerpt, token sweep, and validator has run as a separate output-bounded tool call. Parallel or combined output that truncates is not inspection, even when every command itself succeeded; rerun each affected check separately before mutation.
...
- When browser/manual proof started a process or session, `Proof-process finalization:` records the post-assertion stdout/stderr and browser-console drain plus proof-owned stop/intentional-retention disposition. HMR, stale-process, or unexplained errors keep mutation blocked until a clean final-tree rerun and refreshed body pass; use a reasoned N/A only when no proof-owned process or session was started.
...
- If browser/manual proof started a process or session, finalize and record it before the first mutation. Cleanup deferred only to the final response is not enough because its output can invalidate the evidence being published.
...
rg -n "<[^>\n]{1,120}>" "$body"
wc -c "$body"
rg -n "Closeout gate passed: audit sink|Closeout body check passed|Final SHA:|Verification:|Review:|Review fallback:|Findings:|Axis summary:|TDD evidence gate passed|Principles/ADR conformance:|Local-only SHA:|Evidence identity refresh|Current evidence identities|Superseded evidence identities|Superseded-token sweep" "$body"
...
- When browser/manual proof started any proof-owned process or session, closeout evidence includes a non-blocked `Proof-process finalization:` result captured after the final assertion and before the first tracker mutation.
