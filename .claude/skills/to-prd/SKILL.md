...
Check with the user that these seams match their expectations. If the current conversation already contains an explicit same-session ratification of the exact testing seams, and the user asks to publish or create the intended PRD, treat that as answered seam confirmation; record it in `## Testing Decisions` and `## Further Notes` as answered with the ratified seams. If the confirmation goes unanswered (the user is away), proceed only when the sketch reuses existing seams unchanged — and record in Further Notes that seam confirmation timed out and the seams are open to veto. If the sketch proposes any new seam, stop without publishing and leave the sketch as the turn's deliverable.
